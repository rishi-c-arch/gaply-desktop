//! Low-memory vendored copy of `candle_transformers::models::quantized_qwen2`
//! (candle-transformers v0.11.0, Apache-2.0/MIT — https://github.com/huggingface/candle).
//!
//! ONE functional change from upstream: `token_embd.weight` is kept QUANTIZED
//! (`QMatMul`) and looked up per-row via `QMatMul::embedding()`, instead of
//! upstream's load-time `QTensor::dequantize()` which materializes the whole
//! table as f32. For Qwen2.5-7B that table is 152064×3584×4B ≈ 2.18 GB of f32;
//! quantized it is ~307 MB already counted in the GGUF — the difference is what
//! OOM-killed the perplexity probe on 8 GB machines. `QTensor::embedding`
//! dequantizes only the rows actually indexed, which for our one-token-at-a-time
//! teacher-forcing loop is a single row per forward.
//!
//! Everything else is upstream verbatim (modulo `crate::` → `candle_transformers::`
//! paths). Re-diff against upstream if the candle pin ever moves past 0.11.
...
pub struct ModelWeights {
    // CHANGED from upstream: `Embedding` (dequantized f32 table) → `QMatMul`
    // kept quantized; rows dequantize on lookup in `forward()`.
    tok_embeddings: QMatMul,
...
impl ModelWeights {
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        // CHANGED from upstream: no `.dequantize(device)` — the table stays
        // quantized and `QMatMul::embedding` dequantizes per looked-up row.
        let tok_embeddings =
    ...
    fn hidden_states(&mut self, x: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        // CHANGED from upstream: quantized per-row lookup instead of
        // `Embedding::forward` over a dequantized f32 table.
        let mut layer_in = self.tok_embeddings.embedding(x)?;
        for layer in self.layers.iter_mut() {
            let x = layer_in;
            let residual = &x;
