...
pub const MIGRATIONS: &[Migration] = &[
    ...
    Migration {
        version: 19,
        name: "citation_library_retraction_outcome",
        // Axis C, completed (§11 D102). Migration 11 persisted `retracted` but
        // not the OUTCOME of a check, so an entry the sweep had found clean
        // reverted to "not checked for retraction" on every restart — safe, but
        // it made the sweep unrepeatable and re-prompted over finished work.
        //
        // The CHECK holds the vocabulary at the schema: 'clear' and
        // 'check_failed' are the only outcomes, NULL means never attempted, and
        // a typo cannot quietly become a fourth state that renders as neither.
        // A confirmed retraction is the separate `retracted` column and keeps
        // its priority in the frontend's retractionState.
        //
        // `retraction_checked_at` exists because a persisted 'clear' is a claim
        // with an invisible expiry — retractions happen after a check. Storing
        // when it was established lets the detail row date it. No expiry policy
        // is implied by the column.
        //
        // ADDITIVE + SAFE, the same shape as 11 and 18: both columns nullable,
        // no backfill, no rewrite. Existing rows read back NULL = 'unchecked',
        // which is what they honestly are.
        up: "
            ALTER TABLE citation_library ADD COLUMN retraction_outcome TEXT
                CHECK (retraction_outcome IS NULL
                       OR retraction_outcome IN ('clear', 'check_failed'));
            ALTER TABLE citation_library ADD COLUMN retraction_checked_at INTEGER;
        ",
        down: "
            ALTER TABLE citation_library DROP COLUMN retraction_checked_at;
            ALTER TABLE citation_library DROP COLUMN retraction_outcome;
        ",
    },
    Migration {
        version: 20,
        name: "audit_staged_sources",
        // §11 D132. The author-year reference list's entries, so the audit can
        // fetch and link them WITHOUT writing to `citation_library`.
        //
        // SCOPED TO THE JOB, and cascading with it. 35 entries of a real paper
        // include malformed ones and organisational authors, and the library is
        // shared by every feature — putting them there would mean junk rows in
        // the user's own collection, hard to un-import, for works they never
        // asked to collect. They asked for their manuscript to be checked.
        //
        // `doi_norm` is the dedupe key, stored rather than computed per query so
        // the UNIQUE index can use it: lowercased, `https://doi.org/` stripped,
        // trailing punctuation trimmed. The partial index lets DOI-less entries
        // coexist (they are deduped on `raw` instead) while making a repeated DOI
        // impossible within one job.
        //
        // `matched_by` mirrors `citation_documents`' vocabulary exactly, INCLUDING
        // its CHECK, so "how was this link made" means the same thing on both
        // sides. 'title' is allowed here for piece 2 and is not yet written.
        up: "
            CREATE TABLE audit_staged_sources (
                id          INTEGER PRIMARY KEY AUTOINCREMENT,
                job_id      INTEGER NOT NULL REFERENCES ai_jobs(id) ON DELETE CASCADE,
                ...
                matched_by  TEXT CHECK (matched_by IN ('title','doi','manual')),
                created_at  INTEGER NOT NULL,
                UNIQUE (job_id, raw)
            );
            CREATE UNIQUE INDEX idx_staged_job_doi
                ON audit_staged_sources(job_id, doi_norm) WHERE doi_norm IS NOT NULL;
            CREATE INDEX idx_staged_job ON audit_staged_sources(job_id);
        ",
        down: "
            DROP INDEX IF EXISTS idx_staged_job;
            DROP INDEX IF EXISTS idx_staged_job_doi;
    ...
    Migration {
        version: 24,
        name: "journal_fingerprint_version",
        // Prompt 5 item 6 — versioning and the shared content-addressed cache.
        //
        // ONE ROW PER JOURNAL, holding the version every other journal_* table's
        // rows belong to. The three parts are already separate tables (§7); a
        // version living on each of them could disagree with itself, and then
        // "which fingerprint did this report use" has three answers.
        //
        // `content_hash` is over the SOURCE DOCUMENTS, not over the derived
        // rows. §11 D160's lesson applied to caching: a hash of the output
        // changes whenever the extractor improves, so a re-fetch would be
        // triggered by our own code changing rather than by the journal
        // changing. The question this hash answers is "has the journal moved",
        // and only the sources can answer it.
        //
        // `quarantined_at` / `quarantine_reason`: a profile that fails the
        // injection guards is quarantined FOR ALL USERS (the cache is shared),
        // with the reason kept. A quarantined profile is not deleted — the next
        // fetch needs to know it was quarantined and why, or it will re-ingest
        // the same poisoned page and quarantine it again with no record that
        // this has happened before.
        //
        // `refetch_after`: the quarterly policy, stored as the epoch it becomes
        // due rather than computed at read time. A policy computed in the
        // reader is a policy each reader can get wrong.
        up: "
            CREATE TABLE journal_fingerprints (
        ...
        ",
        down: "
            DROP INDEX idx_journal_fingerprints_due;
